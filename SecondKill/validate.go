...
//设置集群地址，最好内网IP
var hostArray= []string{"169.254.88.170","169.254.88.170"}
...
func (m *AccessControl) GetDistributedRight(req *http.Request) bool {
	...
	hostRequest,err1:=hashConsistent.Get(uid.Value)
	if err1 !=nil {
		return false
	}
...
func Check(w http.ResponseWriter, r *http.Request) {
	...
	if responseValidate.StatusCode == 200 {
		if string(validateBody)=="true" {
			...
			//4.生产消息
			err = mq.Publish(string(byteMessage))
			if err !=nil {
				w.Write([]byte("false"))
				return
			}
...
func CheckUserInfo(r *http.Request) error {
	...
	//对信息进行解密
	signByte, err1 := encrypt.DePwdCode(signCookie.Value)
	if err1 != nil {
...
func main() {
	...
	// 3.1 注册静态模板生成页面
	http.Handle("/html/", http.StripPrefix("/html/",http.FileServer(http.Dir("./web/htmlProductShow"))))
